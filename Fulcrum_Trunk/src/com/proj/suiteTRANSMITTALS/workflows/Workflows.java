...
public class Workflows extends TestSuiteBase{
	...
	public static Hashtable<String,String> Level1_Initaite_Transmittal(WebDriver driver,String url,String worflow_l1,Hashtable<String,String>data) throws Throwable{
		...
		Navigations_Fulcrum.Transmittals.navigateToNewTransmittal(driver,worflow_l1);
		...
		if (transmittalData.size()==0 || transmittalData.get("Subject").equals(Constants_FRMWRK.False)){
		...
		getResult=MyInboxAndActionRequiredPage_Fulcrum.validate_TransmittalID(driver, "Mail-My Sent", worflow_l1, transmittalData);
		transmittalData.put("Mail ID", getResult);

		if(getResult.equalsIgnoreCase(Constants_FRMWRK.False)){
			CustomExceptions.Exit(testcaseName, worflow_l1+"- Failure", "Unable to continue the test due to above error ");
		}

		Transmittals_EntryPage.validateMailedDocumentSentFlag(driver, refID, testcaseName, worflow_l1, data);
		return transmittalData;
	}


	public static WebDriver Level2_Validate_And_DownloadFiles(String siteName,String validationPage,WebDriver driver,String refid,String testcasename,String workflow_l2,String condition,String workflow_end,String browsername,Hashtable<String,String>transmittalData,Hashtable<String,String>testData,int userIteration ) throws Throwable{
		workflow_l2=workflow_l2+condition+" - validate & Download"+workflow_end;

		LinkedHashMap<String,String>actionStatusData=new LinkedHashMap<String,String>();
		...
		if(!testData.get(Constants_Workflow.Fulcrum_WorkFlow_Data_Condition).equalsIgnoreCase(Constants_Workflow.Fulcrum_WorkFlow_Corresponce)){
			actionStatusData=Transmittals_EntryPage.loadRecipientActionStatus(transmittalData,testData.get("Action-Level2"));			
		}

		if(transmittalData.get("To").contains(Constants.delimiter_data)){
			String[] recpients=commonMethods.splitString(transmittalData.get("To"), Constants.delimiter_data);

			for (String recp:recpients){
				LoginLib.logOutFromApplicationAndcloseBrowser(driver,refid,testcasename);
				Users.setLoginUser(recp); //need to update the logic with mutiple recipient
				...
				getResult=MyInboxAndActionRequiredPage_Fulcrum.validate_TxComplete_StatusAndStatus(driver,validationPage, workflow_l2, transmittalData,testData,refid,testcasename,actionStatusData);
				transmittalData.put("Mail Status", getResult);

				TransmittalsGridUtil.searchSubjectAndOpenRecord(driver,validationPage, workflow_l2+"-Download", transmittalData.get("Subject"));
			...
			LoginLib.logOutFromApplicationAndcloseBrowser(driver,refid,testcasename);
			Users.setLoginUser(transmittalData.get("To")); 
			driver=LoginLib.launchBrowserAndlogIntoApplication(browsername,testcasename,TestBase.login_userName,TestBase.login_password,refid);
	...
	public static WebDriver Level2_Validate_And_DownloadFiles_previousversion(String siteName,String validationPage,WebDriver driver,String refid,String testcasename,String workflow_l2,String condition,String workflow_end,String url,String browsername,String username2,String password2,Hashtable<String,String>transmittalData,Hashtable<String,String>testData,int userIteration ) throws Throwable{
		workflow_l2=workflow_l2+condition+" - validate & Download"+workflow_end;
		LoginLib.logOutFromApplicationAndcloseBrowser(driver,refid,testcasename);
		driver=LoginLib.launchBrowserAndlogIntoApplication(browsername,testcasename,username2,password2,refid);

		//commnted on feb 7 2019 need to validate the step	MyInboxAndActionRequiredPage_Fulcrum.validate_TxComplete_StatusAndStatus(driver,validationPage, workflow_l2, transmittalData,testData);
	...
	public static WebDriver Level2_Validate_OR_Submit_OR_ApproveOrReject_OR_Forward_OR_ReplyAll_Transmittal(String siteName,String validationPage,WebDriver driver,String refid,String testcasename,String workflow_l2,String condition,String workflow_end,String url,String browsername,String username2,String password2,Hashtable<String,String>transmittalData,Hashtable<String,String>testData,int userIteration ) throws Throwable{
		transmittalData.put(Constants_Workflow.reciever_counter_key, String.valueOf(userIteration));
		LinkedHashMap<String,String>actionStatusData=new LinkedHashMap<String,String>();

		if(!testData.get(Constants_Workflow.Fulcrum_WorkFlow_Data_RESREQ_Condition).equalsIgnoreCase(Constants_Workflow.Fulcrum_WorkFlow_NONE)){
			actionStatusData=Transmittals_EntryPage.loadRecipientActionStatus(transmittalData,"");			
		}
		if(testData.get(Constants_Workflow.Fulcrum_WorkFlow_Data_Condition).equalsIgnoreCase(Constants_Workflow.Fulcrum_WorkFlow_Corresponce)){
		...
		/* commented on Jan 16 2019	// multi user reject
			...
			driver=ApplicationMethods.launchBrowserAndlogIntoApplication(browsername,testcasename,url,username2,password2,refid);
			getResult=MyInboxAndActionRequiredPage_Fulcrum.validate_TxComplete_StatusAndStatus(driver,validationPage, workflow_l2, transmittalData,testData,refid,testcasename,actionStatusData);
		}*/
		//Forward
		else if(!testData.get(Constants_Workflow.Fulcrum_WorkFlow_Data_RESREQ_Condition).equalsIgnoreCase(Constants_Workflow.Fulcrum_WorkFlow_NONE)&& (testData.get("Action-Level2").equals("Forward"))){
			workflow_l2=workflow_l2+condition+" & "+testData.get("Action-Level2")+workflow_end;	

			//single user- forward from action required
			transmittalData.put(Constants_Workflow.reciever_counter_key, String.valueOf(userIteration));
			String userloginName=transmittalData.get("To");
			Users.setLoginUser(userloginName);

			LinkedHashMap<WebDriver, Hashtable<String,String>>forwardedData=Workflow_reusables.forwardAMail(driver, refid, testcasename, workflow_l2, workflow_end, browsername, transmittalData, testData, actionStatusData, userloginName);

			Map.Entry<WebDriver,Hashtable<String,String>> entry = forwardedData.entrySet().iterator().next();
			driver=entry.getKey();
			//Hashtable<String,String>forwardReturnData=new Hashtable<String,String>();
			transmittalData=entry.getValue();

			String subj=transmittalData.get("Subject");
			if (testData.get("Action-Level2").equalsIgnoreCase("Forward")){
				subj="FW: "+subj;						
				transmittalData.put("Subject", subj);	
				actionStatusData=Transmittals_EntryPage.loadRecipientActionStatus(transmittalData, testData.get("Action-Level2"));
			}				
			getResult=MySentPage.validate_TxComplete_StatusAndStatus(driver, workflow_l2, transmittalData);

			if(getResult.equalsIgnoreCase(Constants_FRMWRK.False)){
				CustomExceptions.Exit(testcasename, workflow_l2+"- Failure", "Unable to continue the test due to above error ");
			}
			getResult=MyInboxAndActionRequiredPage_Fulcrum.validate_TransmittalID(driver, "Mail-My Sent", workflow_l2, transmittalData);
			transmittalData.put("Mail ID", getResult);

			Workflow_reusables.SubmitAnActionOrValidateAAlreadySubmittedAction(driver, refid, testcasename, browsername, workflow_l2, workflow_end, transmittalData, testData, actionStatusData, validationPage, userIteration);


		}

		// Approve or Reject or review or rfi or ack
		else if(!testData.get(Constants_Workflow.Fulcrum_WorkFlow_Data_RESREQ_Condition).equalsIgnoreCase(Constants_Workflow.Fulcrum_WorkFlow_NONE)){
			workflow_l2=workflow_l2+condition+" & "+testData.get("Action-Level2")+workflow_end;		
		...
		/*//Overdue
			...
			LoginLib.logOutFromApplicationAndcloseBrowser(driver,refid,testcasename);
			String userloginName =""; // need to update the logic to pick user.
			driver=LoginLib.launchBrowserAndlogIntoApplication(browsername,testcasename,username2,password2,refid);
			...
			Transmittals_EntryPage.editAndSubmitTransmittalRecord(driver, refID, testcaseName, workflow_l2, transmittalData,testData.get("Action-Level2"),userloginName,actionStatusData);

			validateRecordinActionRequiredPageAfterSubmission(driver,testcasename, validationPage, workflow_l2, transmittalData.get("Tramsmittals-Subject"));
		...
		/*	//Forward
			...
			MyInboxAndActionRequiredPage_Fulcrum.validate_TxComplete_StatusAndStatus(driver,validationPage, workflow_l2, transmittalData,testData,refid,testcasename,actionStatusData);

			Transmittals_EntryPage.verifyAttachedFiles(driver, testcasename, refid, workflow_l2, testData);

			Transmittals_EntryPage.forwardAndSendTransmittalRecord(driver,refid,testcasename, workflow_l2, testData,transmittalData);

			validateRecordinActionRequiredPageAfterSubmission(driver,testcasename, validationPage, workflow_l2, transmittalData.get("Subject"));
	...
	protected static void validateRecordinActionRequiredPageAfterSubmission(WebDriver driver,String testcasename,String validationPage,String workflow,String subject) throws Throwable{
		if(validationPage.equalsIgnoreCase(Constants_Workflow.page_actionRequired)||(validationPage.equalsIgnoreCase(Constants_Workflow.page_actionsOverdue))) {
			...
			getResult=TransmittalsGridUtil.searchSubject(driver,testcasename, validationPage, workflow, subject);
			if(getResult.equalsIgnoreCase(Constants_FRMWRK.False)){
				Reporting.logStep(driver, workflow+"Verification of Action performed record in Action Required page", "The record "+subject+" is not listed after the submission of action", Constants_FRMWRK.Pass);
			}
			else if (getResult.equalsIgnoreCase(Constants_FRMWRK.True)){
				Reporting.logStep(driver, workflow+"Verification of Submission record in Action Required page", "The record "+subject+" is listed after the submission of action", Constants_FRMWRK.Fail);
			}
			else{
