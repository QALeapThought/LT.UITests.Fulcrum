...
public class KeyMethodsUtil extends TestBase{
	...
	public static String js_selectItem(WebDriver driver,String workFlow,String step,String itemsXpath,String input,int type){
		...
		if(elements.size()!=0){
			...
			if(input.equalsIgnoreCase(Constants.any) ){
				flag=elements.get(0).getText();
				try{
				...
				logsObj.log("js_selectItem:- As the input data is Any hence selected first item -"+flag+" and clicked..");
				Reporting.logStep(driver, refID, step, "js_selectItem:- As input is Any first Item listed "+flag+" is located and clicked", Constants_FRMWRK.Pass);
				return flag;
			}
			if(!input.equalsIgnoreCase(Constants.any) ){
				for (WebElement element:elements){
					...
					if(type==1){
						validateCondition=actualtext.equals(input);
					}
					else if (type==2){
						validateCondition=actualtext.contains(input);
					}
	...
	public static String enter_autosuggest_browse(WebDriver driver,String refid,String testcasename,String workFlow,String step,String input,WebElement element) throws Throwable{
		String flag=Constants_FRMWRK.False;
		String generic_autosuggest_step="Select Taxonomy item for ";
		step=workFlow+generic_autosuggest_step+step;
		try{
			element.click();
			ApplicationMethods.switchToLatestDLGframe(driver,testcasename);			
			String isitemSelected=WebTableUtil.selectItemFromTaxonomy(driver, refid,testcasename,workFlow,step, input);		
			if(!isitemSelected.equalsIgnoreCase(Constants_FRMWRK.False)){
				try{
					...
					try{
						...
						ok.click();
						Reporting.logStep(driver, step, "enter_autosuggest_browse: Able to select a choice "+isitemSelected, Constants_FRMWRK.Pass);
						flag=isitemSelected;
	...
	public static String isEnabledWithAttr(WebDriver driver,String refID,String testcaseName,String workFlow,String Step,String locatorType, String objectType, String objectLocator,String input,WebElement element) throws Throwable{
		String flag=Constants_FRMWRK.False;
		...
		try{
			String isEnabled=element.getAttribute("class");
			if(isEnabled.contains("disabled")){
				logsObj.log(testcaseName+"--> "+objectLocator+" exists but disabled and status is "+isEnabled);
				Reporting.logStep(driver, refID, Step,  objectType+": "+objectLocator+" exists but disabled and  status is "+isEnabled, Constants_FRMWRK.Fail);
				isTestPass=Constants_FRMWRK.FalseB;
			}else{
				flag=Constants_FRMWRK.True;
				logsObj.log(testcaseName+"--> "+objectLocator+" exists and enabled ");
				Reporting.logStep(driver, refID, Step,  objectType+": "+objectLocator+" exists and enabled  ", Constants_FRMWRK.Pass);
			}
			...
			String isEnabled=element.getAttribute("class");
			if(String.valueOf(isEnabled).equalsIgnoreCase("disabled")){
				logsObj.log(testcaseName+"--> "+objectLocator+" exists but disabled status -"+isEnabled);
				Reporting.logStep(driver, refID, Step,  objectType+": "+objectLocator+" exists but disabled and status is "+isEnabled, Constants_FRMWRK.Fail);
				isTestPass=Constants_FRMWRK.FalseB;
			}else{
				flag=Constants_FRMWRK.True;
				logsObj.log(testcaseName+"--> "+objectLocator+" exists and enabled");
				Reporting.logStep(driver, refID, Step,  objectType+": "+objectLocator+" exists and enabled  "+String.valueOf(isEnabled), Constants_FRMWRK.Pass);
			}
		}
		return flag;
	}
		...
		try{
			...
			if(isEnabled.contains("disabled")){
				...
				if(ReportResults.equalsIgnoreCase(Constants_FRMWRK.Yes)){
					Reporting.logStep(driver, refID, testcaseName,Step,  objectType+": "+objectLocator+" exists and disabled, status is "+isEnabled, Constants_FRMWRK.Pass);
				}				
				flag=Constants_FRMWRK.True;
			}else{
				isTestPass=Constants_FRMWRK.FalseB;
				logsObj.log(testcaseName+"--> "+objectLocator+" exists and but not disabled & status is "+isEnabled);
				if(ReportResults.equalsIgnoreCase(Constants_FRMWRK.Yes)){
					Reporting.logStep(driver, refID,testcaseName, Step,  objectType+": "+objectLocator+" exists and but not disabled & status is  "+String.valueOf(isEnabled), Constants_FRMWRK.Fail);
				}
