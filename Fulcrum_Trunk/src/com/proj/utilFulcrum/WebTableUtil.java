...
public class WebTableUtil extends TestBase{
	...
	public static List<WebElement> waitUntilAllVisible_old(WebDriver driver, String colXpath){
		int counter =1;
	...
	public static String searchforDataInsearchColumnAndTickInactionableColumn(WebDriver driver,String testcaseName,String Step,String containerName,String 
			searchData,int colToSearch,int actionPerformCol) {
		...
		boolean isdisplayed=ElementPresence.isElementDisplayed(driver, Constants_FRMWRK.FindElementByXPATH, "//table[@summary='"+containerName+"']", Constants_TimeOuts.Element_TimeOut);
...
Sep 7 commented to check below lines					logsObj.log("Checkbox is selected for the record");*/
										if(browserName.equalsIgnoreCase(Constants.browserie)){
											int rcounter=0;
											while (selected==false && rcounter<3){											
												WorkaroundsSelenium.sendControlToElement(browserName, Constants.browserie, row_tick);
												//row_tick.click();
												Clicks.actionsClick2(driver, row_tick);
												selected=row_tick.isSelected();
												rcounter=rcounter+1;
	...
	public static String searchforDataInsearchColumnAndValidateDataInactionableColumn(WebDriver driver,String testcaseName,String Step,String containerName,String searchData,String actionData,int colToSearch,int actionPerformCol) throws Throwable{
		...
		boolean retrieved_matches=Constants_FRMWRK.FalseB;

		String tableContainer_xpath="//table[@summary='containerName']";

		if(containerName.contains(ObjRepository.container_actions)){
			tableContainer_xpath=commonMethods.replaceString("summary",tableContainer_xpath,"id");
		}
		tableContainer_xpath=commonMethods.replaceString("containerName",tableContainer_xpath,containerName);
		//WebElement webtableElement=ElementPresence.elementDisplayed(driver, Constants_FRMWRK.FindElementByXPATH, tableContainer_xpath,Constants_TimeOuts.Element_TimeOut);
		boolean isdisplayed=ElementPresence.isElementDisplayed(driver, Constants_FRMWRK.FindElementByXPATH, tableContainer_xpath, Constants_TimeOuts.Element_TimeOut);
		//if(webtableElement==null){
		if(isdisplayed==false){
		...
		if(containerName.contains(ObjRepository.container_actions)){
			colXpath_search=commonMethods.replaceString("summary",colXpath_search,"id");
			colXpath_action=commonMethods.replaceString("summary",colXpath_action,"id");
		...
		try{
			...
			for (WebElement rowSearch : rows_search) { 
				System.out.println("Action data is --"+actionData);
				...
				if(rowSearch.getText().trim().equals(searchData.trim())){
					...
					try{							
						while (counter< 5){
							try{
								...
								else if(actual_actionData.contains(actionData)){
									retrieved_matches=Constants_FRMWRK.TrueB;
								}
					...
					if(retrieved==Constants_FRMWRK.TrueB){
						flag=Constants_FRMWRK.True;
						Reporting.logStep(driver, Step, "Successfully retrieved data from the required column and  the expected data:-"+actionData+" equals with actual data:-"+actual_actionData+" for search data/record "+searchData+" in the table "+colXpath_search, Constants_FRMWRK.Pass);
					}
					else if (retrieved_matches==Constants_FRMWRK.TrueB){
						flag=Constants_FRMWRK.True;
						Reporting.logStep(driver, Step, "Successfully retrieved data from the required column and  the expected data:-"+actionData+" matches with actual data:-"+actual_actionData+" for search data/record "+searchData+" in the table "+colXpath_search, Constants_FRMWRK.Pass);
					}
					break;
				}
				rowCount++;
			...
			System.out.println("Staleeee ..... at searchforDataInsearchColumnAndValidateDataInactionableColumn");
			searchforDataInsearchColumnAndValidateDataInactionableColumn(driver, testcaseName, Step, containerName, searchData, actual_actionData, colToSearch, actionPerformCol);
		}catch(Exception e){
			isTestPass=Constants_FRMWRK.FalseB;
		...
		if(retrieved==Constants_FRMWRK.FalseB && retrieved_matches==Constants_FRMWRK.FalseB){
	...
	public static String searchforDataInsearchColumnAndClickInactionableLinkColumn(WebDriver driver,String testcaseName,String Step,String containerName,String searchData,String actionColumnType,String actionData,int colToSearch,int actionPerformCol) {
		...
		if(actionColumnType.equalsIgnoreCase("LINK")){
			...
			colXpath_action="//table[@summary='containerName']/descendant :: tr /td[actionPerformCol]/descendant :: a[1]";
		}
						...
						try{							
							while (counter< 5){
								try{
									...
									//WaitUtil.pause(Constants_TimeOuts.generic_TimeOut);
									if(actionColumnType.equalsIgnoreCase("LINK")||actionColumnType.equalsIgnoreCase("DESCENDANT_LINK_1")){
	...
	public static String searchforDataInsearchColumn_WOR(WebDriver driver,String testcaseName,String Step,String containerName,String searchData,int colToSearch) {
		...
		try{
			...
			logsObj.log("Stale Exception occured at searchforDataInsearchColumn_WOR for test case-"+testcaseName+" and step "+Step);
			System.out.println("Stale Exception occured at searchforDataInsearchColumn_WOR for test case-"+testcaseName+" and step "+Step);
			Reporting.logStep(driver, Step+"-Stale Recovery", "Stale Exception occured at searchforDataInsearchColumn_WOR for test case-"+testcaseName+" and step "+Step+" and the error is "+commonMethods.report_stackTrace(stl), Constants_FRMWRK.Warning);
			searchforDataInsearchColumn_WOR(driver, testcaseName, Step, containerName, searchData, colToSearch);
	...
	static boolean clicknextPage(WebDriver driver) {
	...
	public static String selectItemFromTaxonomy(WebDriver driver,String refid,String testcasename,String workflow,String Step,String searchData) throws Throwable{
		String flag=Constants_FRMWRK.False;
		String colXpath_action;
		String rowNumber="row";
		String pageNumber="page";
		...
		String actual_actionData = "";
		boolean clicked=Constants_FRMWRK.FalseB;
		boolean isNextDisplayed=Constants_FRMWRK.FalseB;
		String staleDesc="";
		...
		WebElement webtableElement=ExplicitWaitUtil.waitForElement(driver, Constants_FRMWRK.FindElementByXPATH, ".//table[@id='mainTable']",Constants_TimeOuts.Element_TimeOut);
		if(webtableElement==null){
			Reporting.logStep(driver, Step, staleDesc+"Taxonomy Page- items are not displayed on the page", Constants_FRMWRK.Fail);
			flag=Constants_FRMWRK.False;
		}

		clickPaginationButtons(driver, "Previous");  

		String colXpath_search= ".//table[@id='mainTable']/descendant ::span[contains(@id,'LBL')]";
		colXpath_action= ".//table[@id='mainTable']/descendant ::span[contains(@id,'LBL')]";
		...
		do{
			...
			try{
				logsObj.log("Get Taxonomy items count");
				...
				if (rows_search==null || rows_search.size()== 0)
				{
					Reporting.logStep(driver, Step, staleDesc+"Unable to list the required search data "+searchData+" in the taxonomy table "+colXpath_search+ " As searched data records are not fetched " ,Constants_FRMWRK.Fail);

				}
				if (searchData.equalsIgnoreCase(Constants.any)){
					rows_search.get(1).click();
					dataSelected=rows_search.get(1).getText();
					logsObj.log(staleDesc+"selectItemFromTaxonomy:Successfully clicked the first record from items listed in the taxonomy table") ;
					Reporting.logStep(driver, Step, "Taxonomy Page-"+staleDesc+" As the given data is Any , Successfully clicked the item "+dataSelected+"  from the required taxonomy list", Constants_FRMWRK.Pass);
					flag=dataSelected;
					return flag;
				}
				...
				for (WebElement rowSearch : rows_search) { 
					...
					tableData.put(pageNumber+Integer.toString(pageCount)+" "+rowNumber+Integer.toString(rowCount), rowSearch.getText());
					logsObj.log(staleDesc+"Taxonomy Page- Page-"+pageCount+" ,RowNumber-"+rowNumber+" Data:-"+rowSearch.getText().trim()+" Input Data:-"+searchData.trim());
					...
					if(actual_actionData.equals(searchData.trim())){
						...
						try{	
							...
							while (counter< 5){
								try{
									rows_action.get(rowCount).click();
									dataSelected=rows_action.get(rowCount).getText();
									WaitUtil.pause(1);
									clicked=Constants_FRMWRK.TrueB;
									logsObj.log(staleDesc+"selectItemFromTaxonomy:Successfully clicked the record from items listed in the taxonomy table") ;
									break;
								}catch(StaleElementReferenceException ex){
									counter=counter+1;
									WaitUtil.pause(Constants_TimeOuts.generic_TimeOut);
							...
							isTestPass=Constants_FRMWRK.FalseB;
							Reporting.logStep(driver, Step, "Taxonomy Page-"+staleDesc+"Unable to click the taxonomy item from the required list after successfully matching the expected data in the page"+colXpath_search+" due to error-->"+e+ "and stack is "+commonMethods.getStackTrace(e), Constants_FRMWRK.Fail);

						} 	
						if(clicked==Constants_FRMWRK.TrueB){
							flag=Constants_FRMWRK.True;
							Reporting.logStep(driver, Step, "Taxonomy Page-"+staleDesc+"Successfully Selected the taxonomy item "+dataSelected+" from the list  in the page ", Constants_FRMWRK.Pass);
						}
				...
				isStale=true;
				logsObj.log("selectItemFromTaxonomy:Stale exception... need to recover..") ;
				flag=selectItemFromTaxonomy(driver,refid,workflow, testcasename, Step, searchData);
				isStale=false;
				return flag;
			}
			catch(Exception e){
				isTestPass=Constants_FRMWRK.FalseB;
				Reporting.logStep(driver, Step, "Taxonomy Page-"+staleDesc+"Could not locate the required search data "+searchData+" in the page "+colXpath_search+" due to error-->"+e+ "and stack is "+commonMethods.getStackTrace(e), Constants_FRMWRK.Fail);
				return flag;		
			}
			if(clicked==Constants_FRMWRK.TrueB){
				break;
				...
				pageCount++;
				isNextDisplayed=clickPaginationButtons(driver,"Next");
			}

		}while (isNextDisplayed==Constants_FRMWRK.TrueB && clicked==Constants_FRMWRK.FalseB);
		//
		if(flag.equalsIgnoreCase(Constants_FRMWRK.False)&& isNextDisplayed==Constants_FRMWRK.FalseB){
			isTestPass=Constants_FRMWRK.FalseB;
			Reporting.logStep(driver, Step, "Taxonomy Page-"+staleDesc+"Unable to list the required taxonomy item "+searchData+" in the page "+colXpath_search+ " the complete items listed are: "+tableData, Constants_FRMWRK.Fail);
			Transmittals_EntryPage.clickCancel(driver, refid, testcasename, workflow, "1");
			ApplicationMethods.switchToLatestDLGframe(driver, testcasename);
		}
		else if(clicked==Constants_FRMWRK.FalseB ){
			Reporting.logStep(driver, Step, "Taxonomy Page-"+staleDesc+"Unable to click the taxonomy item from the list  after matching the expected data:-"+searchData+" with actual data:-"+actual_actionData+" in the page "+colXpath_search+" for the search record "+searchData, Constants_FRMWRK.Fail);
			flag=Constants_FRMWRK.False;
	...
	public static boolean clickPaginationButtons(WebDriver driver,String buttonToClick) throws Throwable{
		boolean flag=Constants_FRMWRK.FalseB;
		String locator;
		if(buttonToClick.equalsIgnoreCase("Next")){
			locator=ObjRepository.choice_nxtpage;
		}else{
			locator=ObjRepository.choice_prvpage;
		}

		boolean paginationButtonEnabled=commonMethods.isElementEnabled(driver,Constants_FRMWRK.FindElementByXPATH,locator,Constants_TimeOuts.generic_TimeOut);

		if( paginationButtonEnabled ==true){
			WebElement pagenationButton=ExplicitWaitUtil.waitForElement(driver, Constants_FRMWRK.FindElementByXPATH, locator, Constants_TimeOuts.Element_TimeOut);
