...
public class KeysUtil extends KeyMethods{
	 ...
	 * @throws Throwable
	 */
	protected static String enter_autoSuggestAndSelect(WebDriver driver,String refID,String testcaseName,String workFlow,String Step,String locatorType, String objectType, String objectLocator,String input,WebElement element,int type ) throws Throwable{
		...
		if(!input.equalsIgnoreCase("") ){
			...
			if(input.equalsIgnoreCase("Any")){
				...
				PageLoadWaitUtil.waitForPageToLoad(driver);
				WaitUtil.pause(Constants_TimeOuts.generic_TimeOut);
				String flag_js=KeyMethodsUtil.js_selectItem(driver, workFlow, Step, objectLocator, input,type);
				...
				if(input.contains(Constants.delimiter_data)){
					...
					for (int i=0;i<ip.length;i++){
						...
						PageLoadWaitUtil.waitForPageToLoad(driver);
						WaitUtil.pause(Constants_TimeOuts.generic_TimeOut);
						String flag_js=KeyMethodsUtil.js_selectItem(driver, workFlow, Step, objectLocator_jsitems, ip[i],type);
						if(flag_js.equalsIgnoreCase(Constants_FRMWRK.False)){
							flag=Constants_FRMWRK.False;
						}
					...
					PageLoadWaitUtil.waitForPageToLoad(driver);
					WaitUtil.pause(Constants_TimeOuts.generic_TimeOut);
					String flag_js=KeyMethodsUtil.js_selectItem(driver, workFlow, Step, objectLocator_jsitems, input,type);
					if(flag_js.equalsIgnoreCase(Constants_FRMWRK.False)){
						flag=Constants_FRMWRK.False;
					}
				}
	 ...
	 * @return
	 */
	protected static String dropDown_js(WebDriver driver,String refID,String testcaseName,String workFlow,String Step,String locatorType, String objectType, String objectLocator,String input,WebElement element, int type){
		String flag=Constants_FRMWRK.False;		
		String generic_Step="Select an Item from dropdown ";
		Step=workFlow+generic_Step+Step;
		...
		if(staleflag.equalsIgnoreCase(Constants_FRMWRK.False)){
			...
			WaitUtil.pause(Constants_TimeOuts.generic_TimeOut);
			flag=KeyMethodsUtil.js_selectItem(driver, workFlow, generic_Step, ObjRepository.js_dropdown_items, input,type);
		}
		return flag;
	}
	/**
	...
	protected static String dropdown_autoSuggestAndSelect(WebDriver driver,String refID,String testcaseName,String workFlow,String Step,String locatorType, String objectType, String objectLocator,String input,WebElement element,int type) throws Throwable{
		String flag=Constants_FRMWRK.True;		
		String generic_autosuggest_step="Enter dropdown Suggestion for ";
		generic_autosuggest_step=workFlow+generic_autosuggest_step+Step;
		if(!input.equalsIgnoreCase("") ){
			element.click();
			logsObj.log(testcaseName+"--> "+objectLocator+" exists and the dropdown_autosuggest_jqx is clicked to enter search dropdown item");
			if(!input.equalsIgnoreCase(Constants.any) ){
				String dropdown_searchLocator=objectLocator+"/descendant :: input";
				element=ExplicitWaitUtil.waitForElement(driver,locatorType, dropdown_searchLocator, Constants_TimeOuts.Element_TimeOut) ;
				element.sendKeys(input);
				Reporting.logStep(driver, refID, generic_autosuggest_step, objectType+": "+dropdown_searchLocator+" exists and the value entered is "+input, Constants_FRMWRK.Pass);
			}
			String objectLocator_js_dropdown_items=objectLocator+ObjRepository.js_dropdown_autosuggest_items;
			PageLoadWaitUtil.waitForPagetoLoadCompletely(driver, Constants_TimeOuts.Page_Load_TimeOut);
			PageLoadWaitUtil.waitForAjax(driver);			
			WaitUtil.pause(Constants_TimeOuts.generic_TimeOut);
			String flag_js=KeyMethodsUtil.js_selectItem(driver, workFlow, Step, objectLocator_js_dropdown_items, input,type);
			if(flag_js.equalsIgnoreCase(Constants_FRMWRK.False)){
				flag=Constants_FRMWRK.False;
			}else{
				flag=flag_js;
